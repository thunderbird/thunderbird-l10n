...
looking-for-messages-account = Etsitään uusia viestejä tililtä { $accountName }…
# Feedback message shown in the status bar while we look for new messages in a
# specific folder.
# Variables:
# $folderName (String) - The folder's name.
looking-for-messages-folder = Etsitään uusia viestejä kansiosta { $folderName }…
...
deleting-message =
    { $number ->
        [one] Poistetaan yksi viesti kansiosta { $folderName }…
       *[other] Poistetaan { $number } viestiä kansiosta { $folderName }…
    }
