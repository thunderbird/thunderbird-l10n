...
filter-invalid-custom-header = いずれかのフィルターが ':'、プリント不可能な文字、非 ASCII 文字、8 ビット ASCII 文字などの不正な文字を含むカスタムヘッダーを使用しています。対象のフィルターが含まれている msgFilterRules.dat ファイルを編集して不正な文字をカスタムヘッダーから削除してください。
# Variables:
...
filter-header-label =
    .value = これらのフィルターが自動的に上から順に評価されます。
