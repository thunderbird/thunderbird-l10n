...
policy-DisableChat = チャット機能を無効にします。
policy-DisableCommunity = コミュニティ機能を UI に表示しません (寄付、コミュニティに参加、アイデアを共有など)。
policy-DisableDataCollectionSettings = ユーザーによるデータ収集設定の変更を禁止します。
policy-DisableDefaultClientAgent = デフォルトクライアントエージェントのアクションをすべて禁止します。Windows のみ有効です。他のプラットフォームにはエージェントがありません。
...
policy-DisableFileLink = Filelink 機能を無効にします。
...
policy-DisableMessageForwardingFilters = 自動転送するメッセージにメッセージフィルターを適用しないようにします。
policy-DisablePasswordReveal = 保存されたパスワードの開示を許可しないようにします。
policy-DisableProfileImport = メニューコマンド “他のアプリケーションからデータを読み込む” を無効にします。
policy-DisableExperimentalFeatures = 実験的な機能を無効にします。
policy-DisableQRExport = アカウント設定と認証情報を書き出す QR コードの生成を無効にします。
...
policy-DisableUpdateSettings = アプリケーションの更新設定をユーザーが変更できないようにします。
policy-DisplayMenuBar = メニューバーをデフォルトで表示します。
policy-DNSOverHTTPS = DNS over HTTPS の設定を指示します。
policy-DontCheckDefaultClient = 起動時にデフォルトクライアントか確認する機能を無効にします。
