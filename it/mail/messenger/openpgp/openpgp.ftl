...
openpgp-selection-status-expiring-soon = La configurazione corrente utilizza la chiave <b>{ $key }</b>, che scadrà { $when }. Per mantenere la chiave valida si consiglia di prorogarne ora la data di scadenza.
#   $key (String) - the currently selected OpenPGP key
...
dlg-button-open = &Apri…
dlg-button-save = &Salva
