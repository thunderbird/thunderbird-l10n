...
num-ignored =
    { $count ->
        [one] { $count } ignorována
        [few] { $count } ignorovány
       *[other] { $count } ignorováno
    }
...
messages-total-size = Tyto zprávy zabírají: { $numBytes } #2.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Tyto zprávy zabírají více než { $numBytes }.
...
max-count-exceeded = (Poznámka: Bylo vybráno { $total } zpráv, zobrazeno je prvních { $shown })
...
max-thread-count-exceeded = (Poznámka: Bylo vybráno { $total } vláken, zobrazeno je prvních { $shown })
