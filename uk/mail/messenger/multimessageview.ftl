...
num-ignored =
    { $count ->
        [one] , { $count } ігнороване
        [few] , { $count } ігноровані
       *[many] , { $count } ігнорованих
    }
...
messages-total-size = Це повідомлення займає { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Ці повідомлення займають понад { $numBytes }.
...
max-count-exceeded = (Примітка: Вибрано { $total } повідомлень, показано перші { $shown })
...
max-thread-count-exceeded = (Примітка: Вибрано { $total } розмову, показано перші { $shown })
