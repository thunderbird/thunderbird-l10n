...
num-unread =
    { $count ->
        [one] , { $count } ta o‘qilmagan
       *[other] , { $count } ta o‘qilmagan
    }
...
num-ignored =
    { $count ->
        [one] , { $count } ta e’tiborsiz qoldirilgan
       *[other] , { $count } ta e’tiborsiz qoldirilgan
    }
...
messages-total-size = Ushbu xabarlar { $numBytes } joy oladi.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Ushbu xabarlar { $numBytes } dan ko‘proq joy oladi.
...
max-count-exceeded = (Eslatma: { $total } ta xabar tanlandi, oldindagi { $shown } ta xabar ko‘rsatilgan)
