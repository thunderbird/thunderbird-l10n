...
connection-internet-proxy-legend = Pripojenie na internet a proxy server
...
proxy-autodetect-network =
    .label = Automaticky zisťovať sieťový proxy server
    .accesskey = A
proxy-type-wpad =
...
proxy-type-system =
    ...
    .accesskey = m
proxy-manual-setup =
    .label = Manuálne nastavenie proxy servera:
    .accesskey = M
proxy-type-manual =
...
http-port-label =
    ...
    .accesskey = t
proxy-https-sharing =
    .label = Používať tento proxy server pre zabezpečené webové adresy (HTTPS)
    .accesskey = o
proxy-http-sharing =
...
proxy-auto-script-url =
    .label = Adresa URL skriptu na automatické nastavenie:
    .accesskey = A
proxy-type-auto =
...
no-proxy-addresses-label =
    .value = Pre tieto adresy nepoužívať server proxy:
    .accesskey = r
no-proxy-label =
...
connection-proxy-local-network-desc = Pripojenia v lokálnej sieti (napríklad localhost) nikdy nepoužívajú proxy server.
