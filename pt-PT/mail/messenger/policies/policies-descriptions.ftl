...
policy-DisableCommunity = Não mostrar funcionalidades da comunidade no Interface (tais como Doação, Participar, Partilhar Ideias, etc.)
...
policy-DisableFileLink = Desativar funcionalidade Filelink.
...
policy-DisableMessageForwardingFilters = Impedir que os filtros de mensagens reencaminhem mensagens automaticamente.
...
policy-DisableExperimentalFeatures = Desativar funcionalidades experimentais no Thunderbird.
policy-DisableQRExport = Desativar a geração do código QR para exportar definições da conta e credenciais.
...
policy-DisableUpdateSettings = Impedir o utilizador de alterar as definições de atualização da aplicação.
