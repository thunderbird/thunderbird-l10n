...
account-hub-servers-username-label = 用户名：
account-hub-result-server-label = 服务器：
account-hub-result-port-label = 端口：
account-hub-result-security-label = 安全性：
account-hub-result-authentication-type-label = 身份验证：
account-hub-result-authentication-none = 无
    .title = 无
account-hub-result-authentication-password = 普通密码
    .title = 普通密码
account-hub-result-authentication-encrypted-password = 加密密码
    .title = 加密密码
...
account-hub-result-auth-password = 普通密码
account-hub-result-auth-encrypted-password = 加密密码
...
account-hub-privacy-policy = 隐私政策
account-hub-welcome = 欢迎使用
account-hub-welcome-brand = { -brand-short-name }
account-hub-welcome-text = 欢迎使用 { -brand-short-name }
account-hub-name-warning-icon =
    .title = 请输入您的名字
account-hub-email-label = 电子邮件地址
    .accesskey = E
account-hub-email-input =
    .placeholder = john.doe@example.com
account-hub-email-warning-icon =
    .title = 无效的电子邮件地址
account-hub-password-label = 密码
    .accesskey = P
    .title = 可选，仅用于验证用户名
account-hub-remember-password = 记住密码
    .accesskey = m
account-hub-exchange-label = 您的登录信息
    .accesskey = l
account-hub-installing-addon = 正在下载安装附加组件…
account-hub-success-addon = 已成功安装附加组件
account-hub-success-half-manual = 探测指定服务器找到下列配置：
account-hub-result-security-no-encryption = 未加密
    .title = 未加密
account-hub-result-security-ssl = SSL/TLS
    .title = SSL/TLS
account-hub-result-security-starttls = STARTTLS
    .title = STARTTLS
account-hub-result-no-encryption = 未加密
account-hub-result-ssl = SSL/TLS
account-hub-result-starttls = STARTTLS
account-hub-credentials-wrong = 验证失败。请检查用户名和密码
...
account-hub-exchange-config-unverifiable = 无法验证配置。如果您的用户名和密码确认无误，则可能是服务器管理员已禁用了您为账户所选的配置。请尝试选择其他协议。
account-hub-advanced-setup-button = 高级配置
    .accesskey = A
# Variables:
#  $domain (String): The name of the server where the configuration was found, e.g. rackspace.com.
account-hub-exchange-dialog-question = { -brand-short-name } 在 { $domain } 上找到了您的账户设置信息，您要继续并提交凭据吗？
account-hub-creation-error-title = 创建账户时出错
account-hub-error-server-exists = 收件服务器已存在。
account-hub-confirm-advanced-title = 确认高级配置
account-hub-confirm-advanced-description = 此对话框将关闭，即使配置不正确，也会使用当前设置来创建账户。您确定要继续吗？
account-hub-addon-install-title = 安装
account-hub-encryption-button = 端到端加密
account-hub-address-books-button = 通讯录
account-hub-calendars-button = 日历
...
account-hub-select-security-warning = <span data-l10n-name="error-text">警告：检测到不安全的邮件服务器。</span>此服务器缺少加密措施，将暴露您的密码和数据。请联系管理员升级此连接的安全性，或自担风险并继续操作。详见<a data-l10n-name="error-link">常见问题解答</a>。
...
account-hub-address-book-sync-books-data = { $addressBooks } 个通讯录可用
