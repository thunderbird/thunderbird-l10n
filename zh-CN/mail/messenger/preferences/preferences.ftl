...
recent-destinations-description = 邮件近期移往的文件夹的显示和排序方式选项。
recent-sort-order-label = 排序顺序
    .accesskey = S
recent-sort-order-mru =
    .label = 最近选择在前
recent-sort-order-alphabetic =
    .label = 字母顺序
max-recent-label = 近期文件夹最大数量
    .accesskey = M
...
account-hub-manual-config-checkbox-label =
    .label = 在账户中心中使用新版手动配置流程
    .accesskey = c
account-hub-manual-config-checkbox-description = 账户中心新版手动配置流程
new-calendar-legend = 新版日历事件对话框
new-calendar-checkbox-label =
    .label = 在新版对话框中查看日历事件
new-calendar-checkbox-description = 用于查看事件的实验性新版日历对话框
...
spam-description = 设定默认垃圾邮件设置。若要配置特定账户使用的垃圾邮件设置，可前往“账户设置”进行配置。
spam-marked-label =
    .label = 邮件被标记为垃圾邮件时：
    .accesskey = W
spam-move-label =
    .label = 将其移动到账户的“垃圾邮件”文件夹
    .accesskey = o
spam-delete-label =
    .label = 将其删除
    .accesskey = D
spam-read-description = 在以下情况下，将邮件标记为已读
spam-read-manual-label =
    .label = 被手动标记为垃圾邮件
    .accesskey = M
spam-read-auto-label =
    .label = 被 { -brand-short-name } 确认为垃圾邮件
    .accesskey = T
spam-log-label =
    .label = 启用自适应垃圾邮件过滤记录
    .accesskey = E
...
accent-color-teal =
    .label = 蓝绿色
accent-color-disabled-description = 高对比度模式不支持强调色。
