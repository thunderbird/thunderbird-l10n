...
looking-for-messages-account = Тражење нових порука за { $accountName }…
# Feedback message shown in the status bar while we look for new messages in a
# specific folder.
# Variables:
# $folderName (String) - The folder's name.
looking-for-messages-folder = Тражење нових порука у { $folderName }…
...
deleting-message =
    { $number ->
        [one] Брисање једне поруке из { $folderName }…
        [few] Брисање { $number } поруке из { $folderName }…
       *[other] Брисање { $number } порука из { $folderName }…
    }
