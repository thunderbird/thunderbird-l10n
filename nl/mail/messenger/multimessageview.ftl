...
at-least-num-conversations =
    { $count ->
        [one] { $count }+ conversatie
       *[other] { $count }+ conversaties
    }
# Variables:
...
messages-total-size = Deze berichten nemen { $numBytes } in beslag.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Deze berichten nemen meer dan { $numBytes } in beslag.
