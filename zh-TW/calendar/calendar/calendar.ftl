...
prompt-remove-attachments-title = 移除附加檔案
# LOCALIZATION NOTE (prompt-remove-attachments-text):
#  $count will be replaced with number of attachments
prompt-remove-attachments-text = 您真的想要移除 { $count } 個附件嗎？
...
calendar-task-filter-current =
    .label = 目前的工作
    .accesskey = u
calendar-task-details-title = 標題
calendar-task-details-organizer = 從
calendar-task-details-priority = 優先權
calendar-task-details-priority-low = 低
calendar-task-details-priority-normal = 標準
calendar-task-details-priority-high = 高
calendar-task-details-status = 狀態
calendar-task-details-category = 類別
calendar-task-details-repeat = 重複
calendar-task-details-attachments = 附件
calendar-task-details-start = 開始日
calendar-task-details-due = 到期日
calendar-task-mark-completed =
    .label = 標示為已完成
    .tooltiptext = 將工作標示為已完成
    .accesskey = o
...
calendar-properties-enable-calendar =
    .label = 啟用此行事曆
calendar-properties-provider-missing = 無法找到此行事曆的提供者，此狀況通常發生於您停用或移除某些套件。
...
calendar-publish-publish-dialog-button =
    .buttonlabelaccept = 發佈
calendar-publish-close-button = 關閉
calendar-select-dialog-title = 選取行事曆
...
calendar-extract-event-button =
    .label = 新增為事件
    .tooltiptext = 從訊息中找出行事曆資訊，並新增為您的行事曆事件
...
calendar-auth-enter-user-password-for-calendar = 「{ $calendarName }」({ $location }) 要求您輸入帳號與密碼。
# Variables:
