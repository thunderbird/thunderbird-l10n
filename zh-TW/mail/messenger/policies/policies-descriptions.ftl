...
policy-DisableChat = 停用聊天功能。
policy-DisableCommunity = 不要在介面中顯示社群功能（例如捐款、參與我們、分享點子等）
policy-DisableDataCollectionSettings = 防止使用者更改資料收集選項。
...
policy-DisableFileLink = 停用雲端鏈結功能。
...
policy-DisableMessageForwardingFilters = 防止經由郵件篩選器自動轉寄。
policy-DisablePasswordReveal = 不允許於儲存的登入資訊畫面中顯示密碼。
policy-DisableProfileImport = 停用自其他應用程式匯入資料的選單功能。
policy-DisableExperimentalFeatures = 停用 Thunderbird 的實驗性功能。
policy-DisableQRExport = 不產生可用於匯出帳號設定與登入資訊的 QR Code。
...
policy-DisableUpdateSettings = 防止使用者變更應用程式更新設定。
...
policy-SanitizeOnShutdown2 = 關閉程式時，清除瀏覽資料。
