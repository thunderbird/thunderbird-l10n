...
connection-internet-proxy-legend = 網際網路連線與 Proxy
...
connection-proxy-local-network-desc = 本地網路連線例如 localhost 從不使用 Proxy。
