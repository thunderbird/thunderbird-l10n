...
general-email-notifications-header = 電子郵件通知
...
general-network-and-storage-header = 網路與儲存空間
general-search-performance-label = 搜尋與效能
compose-writing-emails-header = 撰寫郵件
...
compose-message-formatting-title = 郵件格式
compose-address-autocomplete-header = 自動完成地址
...
collection-improve-header = 為了改善 { -brand-short-name } 所收集的資料
collection-community-description = { -brand-short-name } 是由全球社群一同打造。分享匿名的效能資料可幫助我們更快修正 Bug，並為所有人提升軟體速度。
collection-privacy-policy = 閱讀我們的隱私權保護政策
collection-data-sharing-off = <strong>資料分享功能已關閉。</strong>過往的使用資料將於 30 天內從 { -vendor-short-name } 的伺服器永久刪除。
...
collection-share-performance =
    ...
    .accesskey = r
collection-share-performance-description = （包含作業系統、記憶體使用量及功能使用情形等基本技術資訊。{ -brand-short-name } 絕不會收集電子郵件內容、聯絡人或私人訊息。）
...
close-to-tray-label =
    .label = 關閉 { -brand-short-name } 時，移動至工具列
    .accesskey = c
start-in-tray-label =
...
mail-play-button =
    .label = 播放
    .accesskey = P
change-dock-icon-description = 應用程式標記與通知
...
gloda-show-results-as-list-label =
    .label = 預設將全域搜尋結果顯示為清單
    .accesskey = L
datetime-formatting-legend = 日期與時間格式
display-language-legend = 顯示語言
allow-graphics-accel =
    .label = 使用圖形加速功能，讓捲動更順暢並提升效能
    .accesskey = h
...
hide-titlebar-label =
    .label = 隱藏標題列
    .accesskey = H
hide-titlebar-description = 移除視窗名稱列以節省畫面空間
hide-tabbar-single-tab-label =
    .label = 只開啟一個分頁時，隱藏分頁列
    .accesskey = T
auto-hide-tabbar-second-tab-description = 自動隱藏分頁列，開啟第二個分頁起才固定顯示。
system-settings-legend = 系統設定
check-email-app-label =
    .label = 啟動時總是檢查 { -brand-short-name } 是否為預設郵件程式
    .accesskey = A
...
install-updates-automatically-label =
    .label = 自動安裝更新（推薦）
    .accesskey = A
install-updates-automatically-description = 安裝最新的安全性修正，來確保 { -brand-short-name } 的安全。
...
connection-config-description = 控制 { -brand-short-name } 要如何連線到網際網路。
connection-options-button =
    .label = 連線選項…
    .accesskey = S
offline-mode-legend = 離線模式
offline-settings-label = 離線模式設定
...
use-cache-before =
    .value = 最多使用
    .accesskey = U
use-temp-storage-after = MB 空間存放暫存檔案
...
convert-emoticons-label =
    .label = 顯示表情符號圖片
    .accesskey = e
display-text-formatting-label = 引用文字格式
...
dark-mode-checkbox-label =
    .label = 使用暗色模式顯示訊息文字
    .accesskey = d
dark-mode-message-text-description = 對所有收到的訊息用暗色背景顯示，就算寄件者指定使用亮色格式也一樣。
...
dark-message-mode-toggle-label =
    .label = 顯示暗色訊息模式開關
    .accesskey = t
dark-mode-toggle-description = 讓您可在閱讀信件時，快速將畫面切換回亮色模式而無需修改主要設定。
general-folder-settings-header = 信件匣設定
recent-folders-legend = 最近開啟的信件匣
recent-folders-description = 控制移動訊息時，要如何顯示最近使用過的信件匣。
dark-message-mode-toggle-description = 在訊息標頭中，顯示可快速關閉暗色訊息模式的開關
general-folder-options-header = 信件匣管理選項
recent-destinations-legend = 最近選擇的目的地
recent-destinations-description = 讓您可列出並排序最近將訊息移動到哪些信件匣的選項。
recent-sort-order-label = 排序順序：
    .accesskey = S
recent-sort-order-mru =
    .label = 最近選擇的
recent-sort-order-alphabetic =
    .label = 字母順序
max-recent-label = 最近選擇過的信件匣數量上限：
    .accesskey = M
account-hub-legend = 帳號中心
account-hub-checkbox-label =
    .label = 用新版帳號中心註冊帳號
    .accesskey = C
account-hub-checkbox-description = 新版郵件帳號建立流程（實驗中）
account-hub-manual-config-checkbox-label =
    .label = 在帳號中心使用新版手動設定流程
    .accesskey = c
account-hub-manual-config-checkbox-description = 帳號中心新版手動設定流程
new-calendar-legend = 新版行事曆事件對話框
new-calendar-checkbox-label =
    .label = 用新版對話框檢視行事曆事件
new-calendar-checkbox-description = 用新版行事曆對話框檢視事件（實驗中）
account-hub-ab-checkbox-label =
    .label = 用新版帳號中心建立通訊錄
    .accesskey = A
account-hub-ab-checkbox-description = 新版通訊錄建立流程（實驗中）
...
paragraph-spacing-label =
    .label = 按下 Enter 鍵會另起新段落，並增加段落間距
    .accesskey = p
...
compose-format-automatic-option =
    .label = 自動（建議）
compose-format-automatic-description = 若有套用樣式（例如粗體或鏈結），就傳送格式化文字；否則傳送純文字。
compose-format-both-option =
    .label = 同時寄出富格式（HTML）與純文字格式
compose-format-both-description = 總是將兩個版本都寄出，讓收件者的郵件軟體選擇要顯示哪一種。
compose-format-html-option =
    .label = 僅富格式（HTML）文字
compose-format-html-description = 可保留自訂字型、色彩與圖片，但某些舊版的郵件軟體可能無法正確顯示。
compose-format-plain-option =
    .label = 只有純文字
compose-format-plain-description = 移除所有格式、色彩與圖片。最適合簡單的文字郵件，在任何地方都能閱讀。
autocomplete-matches-description = 輸入收件者姓名或電子郵件地址時，從以下位置尋找符合項目：
...
compose-send-automatic-option =
    .label = 自動
compose-send-automatic-description = 若訊息內容未使用樣式，使用純文字格式寄出，否則同時寄出 HTML 與純文字格式，收件者無法顯示 HTML 內容時將改用純文字顯示。
...
collect-outgoing-label =
    .label = 自動將寄信時填寫的收件者地址儲存至：
    .accesskey = A
collect-outgoing-description = 手動建立或從郵件新增聯絡人時的儲存位置。
save-added-contacts-label =
    .value = 將手動新增的聯絡人儲存至：
    .accesskey = m
save-added-contacts-description = 從電子郵件儲存的聯絡人的儲存位置。
show-directory-startup-label =
    .value = 開啟 { -brand-short-name } 時，在通訊錄視窗顯示目錄
    .accesskey = S
...
email-picker-label =
    ...
    .accesskey = A
email-picker-description = 從寄出的訊息中獲得的新聯絡人的儲存位置。
default-add-address-book-label =
    .value = 手動新增聯絡人時的預設通訊錄：
    .accesskey = D
default-add-address-book-description = 從收到的訊息獲得的新聯絡人的儲存位置。
...
exceptions-button =
    .label = 例外網站…
    .accesskey = E
remote-content-privacy-link = 遠端內容對隱私的影響
...
passwords-registered-description = { -brand-short-name } 可以為已註冊的帳號儲存密碼。
view-saved-passwords-button =
    .label = 檢視已存密碼…
    .accesskey = S
primary-password-session-description = 主控密碼可以保護您的密碼，但在每次使用本程式時都必須輸入一次。
...
spam-detected-auto-label =
    .label = 由 { -brand-short-name } 自動偵測到時
    .accesskey = T
spam-log-keep-label =
    .label = 保留垃圾郵件自動偵測紀錄
    .accesskey = E
...
reset-spam-button =
    .label = 重設訓練資料
    .accesskey = R
scam-detection-description = { -brand-short-name } 會檢查訊息當中是否有釣魚鏈結和常見詐騙郵件的跡象。
scam-detection-label =
    .label = 讀到疑似詐騙郵件時警告我
    .accesskey = T
antivirus-check-description = 允許防毒軟體逐筆檢查收到的郵件，再儲存到您的電腦。
antivirus-isolate-label =
    .label = 允許防毒軟體隔離收到的郵件
    .accesskey = A
certificate-verify-description = 當伺服器要求提供憑證來確認您的身分時：
...
certificate-ask-every =
    ...
    .accesskey = A
ocsp-check-label =
    .label = 查詢憑證驗證伺服器（OCSP），以確認憑證是否仍然有效。
    .accesskey = v
...
email-e2ee-automatism-intro = 若您有郵件所有收件者的有效且已接受的金鑰或憑證，{ -brand-short-name } 即可開啟加密。
email-e2ee-turn-on =
    .label = 可用時，自動開啟加密
email-e2ee-turn-off =
...
preferences-doh-setting-automatic =
    .label = 自動保護（建議）
    .accesskey = D
preferences-doh-automatic-desc = { -brand-short-name } 會在可用時自動加密網址查詢。若發生連線問題，或您的 VPN／受管理的網路有此要求，則請切換回標準網路查詢。
preferences-doh-setting-default =
    .label = 預設保護
    .accesskey = D
preferences-doh-default-desc = { -brand-short-name } 會為您決定使用安全 DNS 的時機，以保護您的隱私。
preferences-doh-default-detailed-desc-1 = 在可用安全 DNS 的地區就使用
preferences-doh-default-detailed-desc-2 = 若無法使用安全 DNS 提供者，就使用您的預設 DNS 解析器。
...
preferences-doh-setting-enabled =
    .label = 加強保護
    .accesskey = I
preferences-doh-increased-desc = 總是使用所選的服務提供者。只有在安全服務提供者無法連線時，才改用標準連線。
preferences-doh-enabled-desc = 由您自行決定要使用哪個提供者的安全 DNS。
preferences-doh-enabled-detailed-desc-1 = 使用您選擇的提供者
preferences-doh-enabled-detailed-desc-2 = 只在無法使用安全 DNS 提供者時，使用您的預設 DNS 解析器。
preferences-doh-setting-maximum =
    .label = 最大程度保護
    .accesskey = M
preferences-doh-maximum-desc = 強制要求使用加密 DNS。若安全連線失敗，則無法載入網站。
...
search-preferences-input2 =
    .placeholder = 搜尋選項
    .style = width: 15.4em
managed-by-organization-notice = 您的 { -brand-short-name } 受到組織管理。
...
accent-color-legend = 強調色彩
accent-color-selection-label =
    .value = 選擇主要按鈕與醒目提示的顏色
accent-color-label =
    .value = 指定主要按鈕與強調元素的顏色：
accent-color-os =
    .label = 跟隨作業系統
...
accent-color-teal =
    .label = 鴨綠色
accent-color-disabled-description = 在高對比模式中不支援強調色彩。
...
appearance-radio-card-view =
    .label = 卡片檢視
card-view-options-legend = 卡片檢視選項
...
default-message-list-new-folders-description = 選擇新信件匣的預設排序與討論串選項
...
default-sort-new-messages-top =
    .label = 將最新訊息放在頂端
apply-view-settings-label = 將這些檢視設定套用至：
