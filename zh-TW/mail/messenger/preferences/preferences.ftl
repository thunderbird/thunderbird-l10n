...
spam-detected-auto-label =
    .label = 當 { -brand-short-name } 自動偵測到時
    .accesskey = 當
spam-log-keep-label =
    .label = 保留垃圾郵件偵測自動偵測紀錄
    .accesskey = 保
...
reset-spam-button =
    .label = 重設訓練資料
    .accesskey = R
scam-detection-description = { -brand-short-name } 會檢查訊息當中是否有釣魚鏈結和常見的詐騙郵件。
scam-detection-label =
    .label = 收到詐騙郵件時警告我
    .accesskey = 收
antivirus-check-description = 允許防毒軟體逐筆檢查收到的郵件前再儲存到您的電腦。
...
certificate-ask-every =
    ...
    .accesskey = A
ocsp-check-label =
    .label = 請檢查憑證驗證伺服器 (OCSP) 以確認憑證是否正確。
    .accesskey = 請
...
email-e2ee-automatism-intro = 當郵件中的所有收件者都可以使用已接受的有效金鑰或憑證時{ -brand-short-name } 可以開啟加密。
email-e2ee-turn-on =
    .label = 可用時，自動開啟加密
email-e2ee-turn-off =
    .label = 當收件者變更、無法再加密時自動關閉加密
...
preferences-doh-setting-automatic =
    .label = 自動保護（建議）
    .accesskey = D
preferences-doh-automatic-desc = { -brand-short-name } 會在可進行地址搜尋時自動加密。遇到連線問題或 VPN 網路/受管理的網路需要時切換回標準網路搜尋。
...
preferences-doh-setting-enabled =
    .label = 加強保護
    .accesskey = I
preferences-doh-increased-desc = 總是使用選擇的提供者。僅在安全提供者失效時使用標準連線。
...
preferences-doh-setting-maximum =
    .label = 最大程度保護
    .accesskey = M
preferences-doh-maximum-desc = 嚴格要求 DNS 加密。安全連線失敗後將無法載入網站。
