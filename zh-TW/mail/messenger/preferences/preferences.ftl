...
compose-message-formatting-title = 郵件格式設定
compose-address-autocomplete-header = 地址自動完成
...
allow-graphics-accel =
    .label = 使用圖形加速讓捲動更順暢與效能更棒
    .accesskey = 使
storage-format-label =
    .value = 新帳號的儲存格式:
    .accesskey = 新
mbox-store-single-label =
    .label = 每個資料夾一個檔案 (mbox)
...
hide-titlebar-label =
    .label = 隱藏標題列
    .accesskey = 隱
hide-titlebar-description = 移除顯示視窗名稱的頂端列以節省畫面空間
hide-tabbar-single-tab-label =
    .label = 只開啟一個分頁時自動隱藏分頁列
    .accesskey = 只
auto-hide-tabbar-second-tab-description = 自動隱藏分頁直到您開啟第二個分頁。
system-settings-legend = 系統設定
check-email-app-label =
    .label = 開啟郵件程式時總是檢查 { -brand-short-name } 是否為預設郵件程式
    .accesskey = 開
...
install-updates-automatically-label =
    .label = 自動安裝更新（推薦）
    .accesskey = 自
install-updates-automatically-description = 透過最新的安全性修正修正 { -brand-short-name } 的安全。
...
connection-config-description = 控制 { -brand-short-name } 如何連線到網際網路。
...
use-cache-before =
    .value = 最多使用
    .accesskey = U
use-temp-storage-after = MB 的可存放暫存空間的空間
...
dark-mode-checkbox-label =
    .label = 輸入訊息文字使用暗色模式
    .accesskey = 輸
dark-mode-message-text-description = 對所有收到的訊息使用暗色背景就算寄件者設定的亮色也是如此。
...
dark-message-mode-toggle-label =
    .label = 顯示暗色訊息模式開關
    .accesskey = t
dark-mode-toggle-description = 讓您可在閱讀時快速將郵件切換回亮色模式而不需修改主要設定。
general-folder-settings-header = 資料夾設定
recent-folders-legend = 最近開啟的資料夾
recent-folders-description = 控制移動郵件時最近使用過的資料夾要如何顯示。
...
compose-format-plain-option =
    .label = 只有純文字
compose-format-plain-description = 移除所有格式、色彩與圖片。最適合簡單的文字郵件但可以在任何地方都能使用。
autocomplete-matches-description = 輸入收件者名稱或 E-Mail 時尋找符合下列項目的項目:
...
save-added-contacts-label =
    .value = 儲存手動新增的聯絡人到:
    .accesskey = 儲
save-added-contacts-description = 新聯絡人的儲存位置。
...
search-preferences-input2 =
    .placeholder = 搜尋選項
    .style = width: 15.4em
managed-by-organization-notice = { -brand-short-name } 由組織管理。
...
default-sort-new-messages-top =
    .label = 最新訊息顯示在頂端
apply-view-settings-label = 套用下列畫面設定到:
