...
account-hub-email-find-settings-button = 尋找設定
...
account-hub-servers-username-label = 使用者名稱：
account-hub-result-server-label = 伺服器：
account-hub-result-port-label = Port：
account-hub-result-security-label = 安全性：
account-hub-result-authentication-type-label = 驗證方式：
account-hub-result-authentication-none = 無
    .title = 無
account-hub-result-authentication-password = 普通密碼
    .title = 普通密碼
account-hub-result-authentication-encrypted-password = 加密密碼
    .title = 加密密碼
account-hub-result-authentication-gssapi = Kerberos
    .title = Kerberos
account-hub-result-authentication-ntlm = NTLM
    .title = NTLM
account-hub-result-authentication-external = 外部驗證
    .title = 外部驗證
account-hub-result-authentication-oauth2 = OAuth2
    .title = OAuth2
account-hub-result-auth-none = 無
account-hub-result-auth-password = 普通密碼
account-hub-result-auth-encrypted-password = 加密密碼
account-hub-result-auth-gssapi = Kerberos
account-hub-result-auth-ntlm = NTLM
account-hub-result-auth-external = 外部驗證
...
account-hub-exchange-service-url-label = 輸入服務網址
...
account-hub-result-security-no-encryption = 無加密
    .title = 無加密
...
account-hub-credentials-wrong = 驗證失敗，請檢查輸入的使用者名稱與密碼是否正確
...
account-hub-creation-error-title = 建立帳號時發生錯誤
account-hub-error-server-exists = 收件伺服器已存在。
...
account-hub-same-username-checkbox = 與收件伺服器相同的使用者名稱
...
account-hub-email-protocol-select-notification = 需要輸入其他資訊才能設定此帳號
...
account-hub-result-graph-api-description = 使用 Microsoft Graph API 與 Microsoft 365 同步信件匣與郵件
account-hub-result-graph-shortname = Exchange（Graph）
account-hub-result-graph-description = 使用 Microsoft Graph API 同步信件匣與郵件
...
account-hub-select-security-warning = <span data-l10n-name="error-text">警告：偵測到不安全的郵件伺服器。</span>此伺服器未加密連線，會洩漏您的密碼與資料。請聯絡您的管理員以加密連線，或繼續使用但自負風險。<a data-l10n-name="faq-link">若需更多資訊，請參考 FAQ 常見問題集。</a>
...
address-book-add-remote-carddav-description = 連結 CardDAV 通訊錄
...
address-book-add-ldap-icon2 =
    .alt = 新增 LDAP 通訊錄
address-book-add-ldap-button = 新增 LDAP 通訊錄
...
account-hub-exchange-type-account-type = 帳號類型
account-hub-card-graph-title = Microsoft Graph
account-hub-card-graph-description = Microsoft Graph 是適用於 Microsoft 365 的現代連線方法。
account-hub-card-ews-title = Exchange Web Services
account-hub-card-ews-tag = 傳統
account-hub-card-ews-description = Exchange Web Services (Ews) 可連線到 Microsoft Exchange 以同步您的郵件。
# This is shown after the type description, so "it" is referring to the type (either Microsoft Graph or Exchange Web Services).
account-hub-card-recommended-description = 因為它與您的 Service 網址相符推薦。
account-hub-exchange-type-username = 使用者名稱
account-hub-exchange-type-username-hint = 通常您的電子郵件地址或 Active Directory UPN。
account-hub-exchange-type-authentication = 驗證方式
# Checkbox that can be unchecked to use a custom OAuth config.
account-hub-exchange-type-default-oauth = 使用預設的 Microsoft 365 Copilot 驗證設定
account-hub-exchange-type-oauth-tenant = 商戶 ID
account-hub-exchange-type-oauth-app = 應用程式 ID
account-hub-exchange-type-advanced-config = 進階組態
account-hub-exchange-type-title = 選擇帳號類型
