...
account-hub-email-setup-header = 加入您的電子郵件地址
account-hub-email-setup-incoming = 收件伺服器設定
account-hub-email-setup-outgoing = 寄件伺服器設定
account-hub-email-config-found = 選擇您的電子郵件帳號類型
account-hub-email-enter-password = 請輸入您的電子郵件帳號密碼
account-hub-email-sync-accounts = 同步您的行事歷與通訊錄
