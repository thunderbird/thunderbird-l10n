...
account-hub-same-username-checkbox = 與在收件伺服器使用的使用者名稱相同
...
account-hub-result-graph-api-description = 使用 Microsoft Graph API 對 Microsoft 365 同步信件匣與郵件
...
account-hub-exchange-type-username-hint = 通常是您的電子郵件地址或 Active Directory UPN。
...
account-hub-exchange-type-oauth-tenant = 租用戶 ID
