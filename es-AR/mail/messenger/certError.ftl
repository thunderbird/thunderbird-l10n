...
cert-error-ssl-connection-error = Ocurrió un error conectando a { $hostname }.
# Variables:
#   $hostname (string) - Hostname (and possibly port) of the server with certificate error.
...
cert-error-inline-ssl-connection-error =
    .title = Ocurrió un error conectando a { $hostname }.
...
certificate-checking = Probando conexión con el servidor { $hostname }…
# Variables:
#   $hostname (string) - Hostname (and possibly port) of a server.
certificate-test-success = Se estableció exitosamente una conexión segura con { $hostname }.
# Variables:
#   $hostname (string) - Hostname (and possibly port) of a server.
