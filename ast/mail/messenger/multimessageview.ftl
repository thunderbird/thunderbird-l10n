...
num-unread =
    { $count ->
        [one] , { $count } ensin lleer
       *[other] , { $count } ensin lleer
    }
...
messages-total-size = Estos mensaxes ocupen { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Estos mensaxes ocupen más de { $numBytes }.
...
max-count-exceeded = (Nota: { $total } mensaxes tán seleicionaos, amuésense los { $shown } primeros)
