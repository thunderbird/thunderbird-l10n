...
proxy-type-system =
    ...
    .accesskey = u
proxy-manual-setup =
    .label = Thiết lập proxy thủ công:
    .accesskey = m
proxy-type-manual =
...
http-port-label =
    ...
    .accesskey = p
proxy-https-sharing =
    .label = Sử dụng proxy này cho các địa chỉ web an toàn (HTTPS)
    .accesskey = x
proxy-http-sharing =
...
proxy-auto-script-url =
    .label = URL của tập lệnh thiết lập tự động:
    .accesskey = A
proxy-type-auto =
...
no-proxy-addresses-label =
    .value = Không sử dụng proxy cho các địa chỉ này:
    .accesskey = n
no-proxy-label =
...
connection-proxy-local-network-desc = Các kết nối mạng cục bộ (như localhost) không bao giờ sử dụng proxy.
proxy-auto-login-saved-password =
    .label = Tự động đăng nhập nếu mật khẩu được lưu
    .tooltiptext = Tùy chọn này sẽ tự động xác thực bạn với các proxy khi bạn đã lưu thông tin đăng nhập cho chúng. Bạn sẽ nhận được thông báo nếu quá trình xác thực thất bại.
    .accesskey = i
proxy-socks-remote-dns =
    .label = Định tuyến các yêu cầu tên miền (DNS) thông qua máy chủ proxy SOCKS v5
    .accesskey = d
# Do not translate "localhost", "127.0.0.1/8" and "::1". (You can translate "and".)
