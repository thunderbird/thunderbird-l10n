...
calendar-context-attendance-occurrence-label =
    .value = Lần xuất hiện này
calendar-context-attendance-all-series-label =
    .value = Tất cả các chuỗi sự kiện
...
calendar-auth-enter-user-password-for-calendar = “{ $calendarName }” ({ $location }) đang yêu cầu bạn nhập tên người dùng và mật khẩu.
# Variables:
