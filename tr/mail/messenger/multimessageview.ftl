...
messages-total-size = Bu iletiler { $numBytes } kaplıyor.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Bu iletilerin kapladığı alan: { $numBytes }'den fazla
...
max-count-exceeded = (Not: { $total } ileti seçili, ilk { $shown } tanesi gösteriliyor)
...
max-thread-count-exceeded = (Not: { $total } dizi seçili, ilk { $shown } tanesi gösteriliyor)
