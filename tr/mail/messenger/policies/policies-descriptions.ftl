...
policy-DisableCommunity = Topluluk özelliklerini kullanıcı arayüzünde gösterme (bağış yapma, katkıda bulunma, fikir paylaşma vb. özellikler)
...
policy-DisableFileLink = Filelink özelliğini devre dışı bırak.
...
policy-DisableMessageForwardingFilters = İleti süzgeçlerinin iletileri otomatik olarak iletmesini engelle.
...
policy-DisableExperimentalFeatures = Thunderbird’deki deneysel özellikleri devre dışı bırak.
...
policy-DisableUpdateSettings = Kullanıcının uygulama güncelleme ayarlarını değiştirmesini engelle.
