...
messages-total-size = Diese Nachrichten belegen { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Diese Nachrichten belegen mehr als { $numBytes }.
...
max-count-exceeded = (Hinweis: { $total } Nachrichten sind ausgewählt, die ersten { $shown } werden angezeigt.)
...
max-thread-count-exceeded = (Hinweis: { $total } Themen sind ausgewählt, die ersten { $shown } werden angezeigt.)
