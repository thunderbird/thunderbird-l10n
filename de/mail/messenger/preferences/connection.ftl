...
proxy-type-system =
    ...
    .accesskey = r
proxy-manual-setup =
    .label = Manuelle Proxy-Einrichtung:
    .accesskey = M
proxy-type-manual =
...
http-port-label =
    ...
    .accesskey = p
proxy-https-sharing =
    .label = Diesen Proxy für sichere Web-Adressen (HTTPS) verwenden
    .accesskey = x
proxy-http-sharing =
...
proxy-auto-script-url =
    .label = URL des Skripts für die automatische Einrichtung:
    .accesskey = a
proxy-type-auto =
...
no-proxy-addresses-label =
    .value = Für diese Adressen keinen Proxy verwenden:
    .accesskey = n
no-proxy-label =
...
connection-proxy-local-network-desc = Für lokale Netzwerkverbindungen (z.&thinsp;B. localhost) wird niemals ein Proxy verwendet.
proxy-auto-login-saved-password =
    .label = Automatisch anmelden, wenn ein Passwort gespeichert ist
    .tooltiptext = Beim Aktivieren dieser Einstellungen wird die Anmeldung an Proxys automatisch vorgenommen, falls deren Passwort gespeichert ist. Bei fehlgeschlagener Authentifizierung wird das Passwort vom Benutzer abgefragt.
    .accesskey = A
proxy-socks-remote-dns =
    .label = DNS-Anfragen über den SOCKS-v5-Proxy leiten
    .accesskey = D
# Do not translate "localhost", "127.0.0.1/8" and "::1". (You can translate "and".)
