...
looking-for-messages-account = Buscando mensaxes novas para { $accountName }...
# Feedback message shown in the status bar while we look for new messages in a
# specific folder.
# Variables:
# $folderName (String) - The folder's name.
looking-for-messages-folder = Buscando mensaxes novas en { $folderName }…
...
# $folderName (String) - The folder's name.
deleting-messages = Eliminando { $number } mensaxe(s) de { $folderName }…
