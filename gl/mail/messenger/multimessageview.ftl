...
messages-total-size = Estas mensaxes ocupan { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Estas mensaxes ocupan máis de { $numBytes }.
...
max-count-exceeded = (Nota: { $total } mensaxes seleccionadas, amósanse as { $shown } primeiras)
...
max-thread-count-exceeded = (Nota: { $total } fíos de conversas seleccionados, amósanse os { $shown } primeiros)
