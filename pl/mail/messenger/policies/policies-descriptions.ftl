...
policy-DisableCommunity = Nie pokazuj funkcji społecznościowych w interfejsie użytkownika (takich jak „Przekaż darowiznę”, „Zaangażuj się”, „Udostępnij pomysły” itp.)
...
policy-DisableFileLink = Wyłącz funkcję serwisu przechowywania plików.
...
policy-DisableMessageForwardingFilters = Uniemożliwienie filtrom wiadomości automatycznego przekazywania wiadomości.
...
policy-DisableExperimentalFeatures = Wyłącz funkcje eksperymentalne w programie Thunderbird.
...
policy-DisableUpdateSettings = Uniemożliwienie użytkownikowi zmiany ustawień aktualizacji programu.
