...
openpgp-passphrase-unlock-failed = Nie udało się odblokować klucza ani jego elementów składowych.
...
openpgp-selection-status-expiring-soon = Bieżąca konfiguracja używa klucza <b>{ $key }</b>, który wygaśnie { $when }. Aby zachować ważność klucza, rozważ przedłużenie daty jego wygaśnięcia.
#   $key (String) - the currently selected OpenPGP key
...
key-revoked-revocation-code-compromised = Klucz został unieważniony, ponieważ ujawniono jego treść.
