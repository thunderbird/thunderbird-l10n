...
num-ignored =
    { $count ->
        [one] { $count } zignorowana
        [few] { $count } zignorowane
       *[many] { $count } zignorowanych
    }
...
messages-total-size = Zaznaczone wiadomości zajmują { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Zaznaczone wiadomości zajmują ponad { $numBytes }.
...
max-count-exceeded = (zaznaczone wiadomości: { $total }, wyświetlane tylko: { $shown })
...
max-thread-count-exceeded = (zaznaczone wątki: { $total }, wyświetlane tylko: { $shown })
