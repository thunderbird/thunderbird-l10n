...
at-least-num-conversations =
    { $count ->
        [one] Plus d’une conversation
       *[other] Plus de { $count } conversations
    }
# Variables:
...
at-least-num-ignored =
    { $count ->
        [one] , plus d’un ignoré
       *[other] , plus de { $count } ignorés
    }
...
messages-total-size = La taille de ces messages est de { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = La taille de ces messages est de plus de { $numBytes }.
...
max-count-exceeded = (Note : { $total } messages sont sélectionnés, les { $shown } premiers sont affichés)
...
max-thread-count-exceeded = (Note : { $total } discussions sont sélectionnées, les { $shown } premières sont affichées)
