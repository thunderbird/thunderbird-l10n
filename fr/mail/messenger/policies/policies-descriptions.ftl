...
policy-DisableChat = Désactiver la fonctionnalité de messagerie instantanée.
policy-DisableCommunity = Ne pas afficher les fonctionnalités communautaires dans l’interface (telles que faire un don, s’impliquer, partager des idées, etc.)
policy-DisableDataCollectionSettings = Empêcher l’utilisateur de modifier les paramètres de collecte de données.
...
policy-DisableFileLink = Désactiver la fonctionnalité Filelink.
...
policy-DisableMessageForwardingFilters = Empêcher les filtres de messages de transférer automatiquement les messages.
...
policy-DisableExperimentalFeatures = Désactiver les fonctionnalités expérimentales dans Thunderbird.
...
policy-DisableUpdateSettings = Empêcher l’utilisateur de modifier les paramètres de mise à jour des applications.
...
policy-InAppNotification = Activer ou désactiver des types de notifications.
