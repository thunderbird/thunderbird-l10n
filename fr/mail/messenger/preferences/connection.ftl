...
connection-internet-proxy-legend = Connexion Internet et proxy
connection-proxy-legend = Configuration du serveur proxy pour accéder à Internet
proxy-type-no =
    .label = Pas de proxy
    .accesskey = d
proxy-autodetect-network =
    .label = Détecter automatiquement le proxy réseau
    .accesskey = D
proxy-type-wpad =
...
http-port-label =
    ...
    .accesskey = p
proxy-https-sharing =
    .label = Utiliser ce proxy pour les adresses web sécurisées (HTTPS)
    .accesskey = U
proxy-http-sharing =
...
proxy-auto-script-url =
    .label = URL du script d’installation automatique :
    .accesskey = U
proxy-type-auto =
...
no-proxy-addresses-label =
    .value = Ne pas utiliser de proxy pour ces adresses :
    .accesskey = N
no-proxy-label =
...
connection-proxy-local-network-desc = Les connexions au réseau local (comme localhost) n’utilisent jamais de proxy.
proxy-auto-login-saved-password =
    .label = Connexion automatique si un mot de passe est enregistré
...
proxy-socks-remote-dns =
    .label = Acheminer les requêtes de domaine (DNS) via le proxy SOCKS v5
    .accesskey = A
# Do not translate "localhost", "127.0.0.1/8" and "::1". (You can translate "and".)
