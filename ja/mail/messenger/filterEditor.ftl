...
filter-invalid-custom-header = いずれかのフィルターが ':'、印刷不可能な文字、非 ASCII 文字、8 ビット ASCII 文字などの不正な文字を含むカスタムヘッダーを使用しています。対象のフィルターが含まれている msgFilterRules.dat ファイルを編集して不正な文字をカスタムヘッダーから削除してください。
# Variables:
