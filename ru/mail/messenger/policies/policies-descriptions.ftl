...
policy-DisableChat = Отключает функцию чата.
policy-DisableCommunity = Не показывать в пользовательском интерфейсе функции сообщества (такие как «Пожертвовать», «Принять участие», «Поделиться идеями» и т.д.)
...
policy-DisableFileLink = Отключает функцию Filelink.
...
policy-DisableMessageForwardingFilters = Запрещает фильтрам автоматически пересылать сообщения.
...
policy-DisableUpdateSettings = Не позволяет пользователю изменять параметры обновления приложения.
...
policy-InAppNotification = Включает или отключает типы уведомлений.
