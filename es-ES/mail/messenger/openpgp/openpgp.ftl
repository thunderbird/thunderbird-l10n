...
e2e-csr-intro = Para obtener un nuevo certificado S/MIME personal, tiene que generar una solicitud de firma de certificado (CSR) y enviarla a una autoridad de certificación (CA).
# Do not translate: S/MIME, CSR
e2e-csr-intro-info = Seleccione un directorio local y un nombre de archivo para el archivo CSR y responda las siguientes preguntas para configurar el algoritmo y la fuerza.
...
openpgp-selection-status-expiring-soon = Su configuración actual usa la clave <b>{ $key }</b>, que caducará { $when }. Para mantener la validez de la clave, puede ampliar su fecha de caducidad ahora.
#   $key (String) - the currently selected OpenPGP key
...
key-revoked-revocation-code-superseded = La clave ha sido revocada porque está obsoleta.
key-revoked-revocation-code-compromised = La clave ha sido revocada porque los elementos de la clave se han visto comprometidos.
key-revoked-revocation-code-retired = La clave ha sido revocada porque se ha retirado y ya no se utiliza.
# Variables:
# $reason (String) - Key owner entered reason for revocation.
key-revoked-revocation-reason = Motivo de la revocación indicado por el propietario de la clave: { $reason }
