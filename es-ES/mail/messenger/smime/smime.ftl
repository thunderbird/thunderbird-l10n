...
configured-cert-ok-enc = El certificado pasó la prueba y puede usarse para cifrar correo electrónico.
configured-cert-ok-sig = El certificado pasó la prueba y se puede utilizar para firmar correos electrónicos.
...
smime-crypto-digest-algorithm = Algoritmo de resumen de firmas:
...
configured-cert-failure-use-anyway-detail-v2 =
    ...
    La verificación del certificado ha fallado con el siguiente error: { $errorMsg } (Código de error: { $errorCodeStr })
    
    ¿Quiere utilizar este certificado de todos modos?
# $errorCode A numeric error code
# Note the initial empty line is necessary to preserve the line breaks.
configured-cert-failure-use-anyway-v2 =
    Hay un problema con este certificado.
    
    La verificación del certificado ha fallado con el código de error { $errorCode }.
    
    ¿Deseas utilizar este certificado de todos modos?
