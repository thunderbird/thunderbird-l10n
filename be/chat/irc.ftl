...
connection-error-time-out = Часаспын злучэння
...
options-quit-message = Закрыць паведамленне
...
ctcp-time = Мясцовы час { $username }: { $timeResponse }.
...
command-ban = { $commandName } &lt;нік!user@host&gt;: Блакаваць карыстальнікаў, якія адпавядаюць гэтаму шаблону.
# $commandName is the command name
...
# $commandName is the command name
command-mode-user2 = { $commandName } &lt;нік&gt; [(+|-)&lt;рэжым&gt;]: Атрымаць, усталяваць ці зняць рэжым карыстальніка.
# $commandName is the command name
command-mode-channel2 = { $commandName } [&lt;канал&gt;] [(+|-)&lt;новы рэжым&gt; [&lt;параметр&gt;][,&lt;параметр&gt;]*]: Атрымаць, усталяваць ці зняць рэжым канала.
# $commandName is the command name
...
message-ping =
    { $delay ->
        [one] Час адказу ад { $source } — { $delay } мілісекунда.
        [few] Час адказу ад { $source } — { $delay } мілісекунды.
       *[many] Час адказу ад { $source } — { $delay } мілісекунд.
    }
...
#    $name is the nickname or channel name that isn't available.
error-unavailable = { $name } часова недаступны.
#    $name is the channel name.
error-channel-banned = Вы былі заблакаваны на { $name }.
#    $name is the channel name.
error-cannot-send-to-channel = You cannot send messages to { $name }.
#    $name is the channel name.
error-channel-full = Канал { $name } перапоўнены.
#    $name is the channel name.
error-invite-only = Для далучэння да { $name } вы павінны быць запрошаныя.
#    $name is the channel name.
...
#    he was forwarded to.
error-channel-forward = Вы не можаце далучыцца да { $name } і былі аўтаматычна пераадрасаваны на { $details }.
#    $mode is the mode that the user tried to set but was not recognized
#    by the server as a valid mode.
error-unknown-mode = '{ $mode }' не з'яўляецца сапраўдным рэжымам карыстальніка на серверы.
...
#    protect users).
tooltip-connected-from = Злучанне ад
tooltip-registered = Registered
tooltip-registered-as = Зарэгістраваны як
