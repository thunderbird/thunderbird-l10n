...
messages-total-size = Aceste mesaje ocupă { $numBytes }.
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = Aceste mesaje ocupă mai mult de { $numBytes }.
...
max-count-exceeded = (Notă: sunt selectate { $total } mesaje, sunt afișate primele { $shown })
...
max-thread-count-exceeded = (Notă: sunt selectate { $total } conversații, sunt afișate primele { $shown })
