...
connection-error-server-closed-connection = Tenaren lèt att tilkoplinga
...
connection-error-start-tls-required = Tenaren krev kryptering, men du har slått det av
connection-error-start-tls-not-supported = Tenaren støttar ikkje kryptering, men konfigurasjonen din krev det.
connection-error-failed-to-start-tls = Klarte ikkje å starta kryptering
connection-error-no-auth-mec = Ingen autentiseringsmåtar vert tilbydde av tenaren.
connection-error-no-compatible-auth-mec = Ingen av autentiseringsmåtane som tenaren tilbyr er støtta.
connection-error-not-sending-password-in-clear = Tenaren støttar berre autentisering ved å sende passordet i klartekst.
...
connection-error-xmpp-not-supported = Denne tenaren støttar ikkje XMPP
...
conversation-error-join-failed-remote-server-not-found = Klarte ikkje å kopla til rommet { $mucName }, fordi tenaren rommet er styrt frå ikkje kunne bli nådd.
...
conversation-error-remote-server-not-found = Klarte ikkje å finna tenaren til mottakaren
...
command-join3 = { $commandName } [&lt;rom&gt;[@&lt;server&gt;][/&lt;kallenamn&gt;]] [&lt;passord&gt;]: Kopla til eit rom, eller spesifiser ein annan tenar, skallenamn eller passord for rpmmet.0
# $commandName (String): command name
