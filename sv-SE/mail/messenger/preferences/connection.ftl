...
connection-internet-proxy-legend = Internetanslutning och proxy
connection-proxy-legend = Ange proxy för anslutning till Internet
proxy-type-no =
    .label = Ingen proxy
    .accesskey = I
proxy-autodetect-network =
    .label = Automatiskt identifiera nätverksproxy
    .accesskey = A
proxy-type-wpad =
...
proxy-type-system =
    ...
    .accesskey = d
proxy-manual-setup =
    .label = Manuell proxyinställning:
    .accesskey = M
proxy-type-manual =
...
http-port-label =
    ...
    .accesskey = P
proxy-https-sharing =
    .label = Använd denna proxy för säkra webbadresser (HTTPS)
    .accesskey = x
proxy-http-sharing =
...
proxy-auto-script-url =
    .label = Skriptadress för automatisk installation:
    .accesskey = S
proxy-type-auto =
...
no-proxy-addresses-label =
    .value = Använd inte en proxy för dessa adresser:
    .accesskey = n
no-proxy-label =
...
connection-proxy-local-network-desc = Lokala nätverksanslutningar (som localhost) använder aldrig en proxy.
proxy-auto-login-saved-password =
    .label = Logga in automatiskt om ett lösenord är sparat
    .tooltiptext = Det här alternativet autentiserar dig automatiskt till proxyservrar när du har sparat inloggningar för dem. Du tillfrågas endast om autentiseringen misslyckas.
    .accesskey = L
proxy-socks-remote-dns =
    .label = Dirigera domänförfrågningar (DNS) genom SOCKS v5-proxy
    .accesskey = D
# Do not translate "localhost", "127.0.0.1/8" and "::1". (You can translate "and".)
