...
policy-DisableBuiltinPDFViewer = { -brand-short-name }에 내장된 PDF 뷰어인 PDF.js를 비활성화합니다.
...
policy-DisableCommunity = (기부, 참여, 아이디어 공유 등) 커뮤니티 기능을 UI에 표시하지 않습니다.
policy-DisableDataCollectionSettings = 사용자가 데이터 수집 설정을 변경하지 못하도록 합니다.
...
policy-DisableFileLink = 파일 공유 기능을 사용하지 않습니다.
...
policy-DisableMessageForwardingFilters = 메시지 필터가 자동으로 메시지를 전달하지 않게 합니다.
...
policy-DisableExperimentalFeatures = Thunderbird의 실험적인 기능을 끕니다.
policy-DisableQRExport = 계정 설정 및 자격 증명을 내보내기 위한 QR 코드 생성을 비활성화합니다.
...
policy-DisableUpdateSettings = 사용자가 애플리케이션 업데이트 설정을 변경하지 못하도록 합니다.
