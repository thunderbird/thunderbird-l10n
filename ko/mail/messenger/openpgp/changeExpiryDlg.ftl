...
info-explanation-1-complex = 이 OpenPGP 키는 기본 키와 <b>만료 날짜가 서로 다른</b> 하나 이상의 하위 키로 구성됩니다.
select-key-prompt = 변경할 키:
...
partial-label-expired = 만료됨
partial-label-never-expires = 만료되지 않음
# Variables:
# $date (String) - Date the key is expiring on.
partial-label-expires = 만료일: { $date }
