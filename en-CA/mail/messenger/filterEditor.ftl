...
filter-failure-warning-prefix = Filter action failed: “{ $errorMsg }” with error code={ $errorCode } while attempting:
...
filter-log-match-summary = Applied filter “{ $filterName }” to message from { $author } - { $subject } at { $date }
# Variables:
