...
num-conversations = { $count } การสนทนา
# Variables:
...
at-least-num-ignored = , { $count }+ ถูกเพิกเฉย
...
messages-total-size = ข้อความเหล่านี้ใช้เนื้อที่ { $numBytes }
# A message indicating the total size on disk of the selected messages. We use
# this when we didn't actually scan the entire list of selected messages, so
# this is a *minimum* size.
# Variables:
#   $numBytes - the size, e.g. "123 KB".
messages-total-size-more-than = ข้อความเหล่านี้ใช้เนื้อที่มากกว่า { $numBytes }
...
max-count-exceeded = (หมายเหตุ: { $total } ข้อความถูกเลือกอยู่ จะแสดง { $shown } ข้อความแรกก่อน)
...
max-thread-count-exceeded = (หมายเหตุ: { $total } เธรดถูกเลือกอยู่ จะแสดง { $shown } ข้อความแรกก่อน)
