...
xpinstall-prompt-message = Tu tenta de installar un additivo ex { $host }. Verifica que tu fide in iste sito ante continuar.
...
addon-local-install-error-file-access = { $addonName } non pote esser installate perque { -brand-short-name } non pote modificar le file necessari.
