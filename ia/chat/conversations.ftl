...
target-changed = Le conversation continuara con { $targetName }, per { $targetProtocol }.
...
#  $displayName is the display name of the contact.
#  $statusType is the new status type (a value from status.ftl).
status-known = Tu conto ha essite reconnectite ({ $displayName } es { $statusType }).
#  $displayName is the display name of the contact.
#  $statusType is the new status type (a value from status.ftl).
#  $statusText is the status text (eg. "I'm currently away from the computer").
status-known-with-status-text = Tu conto ha essite reconnectite ({ $displayName } es { $statusType }: { $statusText }).
# LOCALIZATION NOTE (status-unknown):
#  $displayName is the display name of the contact.
status-unknown = Tu conto es disconnectite (le stato de { $displayName } non es plus note).
account-disconnected = Tu conto es disconnectite.
account-reconnected = Tu conto ha essite reconnectite.
# LOCALIZATION NOTE (auto-reply):
#  $message is replaced by the text of a message that was sent as an automatic reply.
auto-reply = Responsa automatic - { $message }
# LOCALIZATION NOTE (no-topic-key):
# Displayed instead of the topic when no topic is set.
no-topic-key = Nulle argumento definite pro iste camera.
# LOCALIZATION NOTE (topic-set):
#  $conversationName is the conversation name, $topic is the topic.
topic-set = Le argumento pro { $conversationName } es: { $topic }.
# LOCALIZATION NOTE (topic-not-set):
#  $conversationName is the conversation name.
topic-not-set = Il ha nulle argumento pro { $conversationName }.
# LOCALIZATION NOTE (topic-changed):
#  $user is the user who changed the topic, $topic is the new topic.
topic-changed = { $user } ha cambiate le argumento a: { $topic }.
# LOCALIZATION NOTE (topic-cleared):
#  $user is the user who cleared the topic.
topic-cleared = { $user } ha vacuate le topico.
# LOCALIZATION NOTE (nick-set-key):
#   This is displayed as a system message when a participant changes his/her
#   nickname in a conversation.
#   $oldNick is the old nick.
#   $newNick is the new nick.
nick-set-key = { $oldNick } es ora appellate { $newNick }.
# LOCALIZATION NOTE (nick-set-you):
#   This is displayed as a system message when your nickname is changed.
#   $newNick is your new nick.
nick-set-you = Tu es ora note como { $newNick }.
